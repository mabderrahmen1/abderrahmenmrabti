    ...
    en: {
        ...
        projects: {
            ...
            items: [
                ...
                { title: "Chkobba Game", desc: "Responsive online Tunisian card game built with a friend, featuring the classic Chkobba and Rummy games playable in the browser.", img: "img/chkobba.png", tech: ["React", "Node.js", "Socket.IO", "TypeScript", "Tailwind CSS", "Docker"], github: "https://github.com/souhail-lass/chkobba", live: "https://chkobba.app/" }
            ]
        },
        blog: {
    ...
    fr: {
        ...
        projects: {
            ...
            items: [
                ...
                { title: "Jeu Chkobba", desc: "Jeu de cartes tunisien en ligne responsive développé avec un ami, avec la Chkobba classique et le Rummy jouables dans le navigateur.", img: "img/chkobba.png", tech: ["React", "Node.js", "Socket.IO", "TypeScript", "Tailwind CSS", "Docker"], github: "https://github.com/souhail-lass/chkobba", live: "https://chkobba.app/" }
            ]
        },
        blog: {
...
function openProject(p) {
    ...
    const liveBtn = p.live ? `<a href="${p.live}" target="_blank" class="cv-btn" style="margin-top: 2rem; margin-left: 1rem; display: inline-flex; align-items: center; gap: 0.5rem;"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg> Live Demo</a>` : '';
    const techTags = p.tech.map(t => `<span class="skill-tag">${t}</span>`).join('');
    overlayBody.innerHTML = `<h1>${p.title}</h1><img src="${p.img}" alt="${p.title}" style="width: 100%; border-radius: 8px; margin-bottom: 2rem; border: 1px solid var(--accent-color);"><p style="font-size: 1.1rem; margin-bottom: 2rem; color: var(--secondary-color);">${p.desc}</p><h3 style="color: var(--accent-color); margin-bottom: 1rem;">Technologies</h3><div class="skills-grid">${techTags}</div>${githubBtn}${liveBtn}`;
